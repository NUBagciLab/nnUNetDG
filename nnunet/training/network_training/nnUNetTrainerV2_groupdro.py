...
class nnUNetTrainerV2_groupdro(nnUNetTrainer):
    """
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            raise NotImplementedError("fp16 is not implemented for group DRO")
        else:
            output = self.network(data)
            del data
            l = self.loss(output, target)
            if self.q is None or not self.q.numel():  
                self.q = to_cuda(torch.ones(1)) if torch.cuda.is_available() else torch.ones(1)
            self.q *= (1e-2 * l.data).exp()
            self.q /= self.q.sum()
            l = torch.dot(l.unsqueeze(0), self.q)
            if do_backprop:
                l.backward()
