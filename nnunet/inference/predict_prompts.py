    ...
    for preprocessed in preprocessing:
        ...
        if use_box_prompt or use_point_prompt:
            for i in range(3):
                ...
                left_patch = (min_pos[i] + max_pos[i]) // 2 - patch_size[i] // 2
                # left_patch = min_pos[i] - patch_size[i] // 2
                left_patch = max(0, min(left_patch, min_pos[i]))
                right_patch = (min_pos[i] + max_pos[i]) // 2 + patch_size[i] // 2
                # right_patch = max_pos[i] + patch_size[i] // 2
                right_patch = min(data[0].shape[i], max(right_patch, max_pos[i]))
        ...
        # original_shape = data_crop[0].shape
        ...
        # # data_resized, seg_resized = resize_multichannel_image(data_crop, patch_size), \
        # #                            np.expand_dims(resize_segmentation(seg_crop[0], patch_size, 0), 0)
        seg_resized = (seg_resized > 0.5).astype(np.int64)
        # print("Volume before resize: ", np.sum(seg_crop[0]>0.5))
        # print("Volume after resize: ", np.sum(seg_resized[0]>0.5))
        ### build segmentation box and points prompts

        # data_resized = torch.from_numpy(data_resized).to(torch.float32)
        # seg_resized = torch.from_numpy(seg_resized>0.5).to(torch.int64)
